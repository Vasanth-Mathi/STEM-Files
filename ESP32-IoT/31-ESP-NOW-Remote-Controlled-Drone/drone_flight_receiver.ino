/*
  ...
  Receives control commands through ESP-NOW, reads an MPU6050 IMU,
  estimates roll and pitch, and drives four brushed motors through
  four separate MOSFET motor-driver stages.
...
bool readMpuRaw(
  ...
  const int received = Wire.requestFrom(
...
bool initializeMotorPwm() {
  const bool motor1Ready = ledcAttach(
    MOTOR_1_PIN,
    MOTOR_PWM_FREQUENCY,
    MOTOR_PWM_RESOLUTION
  );

  const bool motor2Ready = ledcAttach(
    MOTOR_2_PIN,
    MOTOR_PWM_FREQUENCY,
    MOTOR_PWM_RESOLUTION
  );

  const bool motor3Ready = ledcAttach(
    MOTOR_3_PIN,
    MOTOR_PWM_FREQUENCY,
    MOTOR_PWM_RESOLUTION
  );

  const bool motor4Ready = ledcAttach(
    MOTOR_4_PIN,
    MOTOR_PWM_FREQUENCY,
    MOTOR_PWM_RESOLUTION
  );
...
void setup() {
  ...
  Serial.print("Drone ESP32 MAC: ");
  Serial.println(WiFi.macAddress());
  Serial.println("Flight controller ready.");
  Serial.println("Keep propellers removed for first tests.");
}
...
void loop() {
  ...
  if (
    commandTime == 0 ||
    millis() - commandTime > FAILSAFE_MS
  ) {
    disarmDrone();
    return;
  }
  ...
  rollAngle =
    ...
    (rollAngle + gx * dt) +
    (1.0f - COMPLEMENTARY_GYRO_WEIGHT) *
    accelRoll;
  ...
  const float targetRoll =
    command.roll * MAX_TILT_ANGLE_DEG / 100.0f;

  const float targetPitch =
    command.pitch * MAX_TILT_ANGLE_DEG / 100.0f;

  const float targetYawRate =
    command.yaw * MAX_YAW_RATE_DPS / 100.0f;
  ...
  float yawCorrection =
    YAW_RATE_KP * yawRateError;

  rollCorrection = constrain(
    rollCorrection,
    -MAX_PID_CORRECTION,
    MAX_PID_CORRECTION
  );

  pitchCorrection = constrain(
    pitchCorrection,
    -MAX_PID_CORRECTION,
    MAX_PID_CORRECTION
  );

  yawCorrection = constrain(
    yawCorrection,
    -MAX_PID_CORRECTION,
    MAX_PID_CORRECTION
  );

  const float baseThrottle = constrain(
    static_cast<float>(command.throttle),
    0.0f,
    static_cast<float>(MAX_BASE_THROTTLE)
  );
